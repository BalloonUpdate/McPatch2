...
fn main() -> ProcessResult {
    ...
    match task.as_deref() {
        Some("client") => dist_binary("mcpatch-client"),
        Some("manager") => dist_binary("mcpatch-manager"),
        _ => print_help(),
    }
}

fn dist_binary(crate_name: &str) -> ProcessResult {
    std::env::set_var("RUST_BACKTRACE", "1");

    let ref_name = github_ref_name();
    let dist_dir = project_root().join("target/dist");
    let target = TargetInfo::get(crate_name, &ref_name, &dist_dir);
    ...
    let status = Command::new(cargo)
        ...
        .args(&["build", "--release", "--bin", crate_name, "--target", &target.rustc_target])
        .status()?;
...
impl TargetInfo {
    fn get(crate_name: &str, version_label: &str, dist_dir: &Path) -> Self {
        let rustc_target = std::env::var("MP_RUSTC_TARGET").unwrap();
        ...
        let symbols_name_versioned = symbols_suffix.map(|e| format!("{crate_name}-{version_label}-{rustc_target}{e}"));

        let artifact_name = format!("{crate_name}{exe_suffix}");
        let artifact_name_versioned = format!("{crate_name}-{version_label}-{rustc_target}{exe_suffix}");
